...
func NewDownload(url string, therads int) Download {
	return Download{url: url, threads: therads, outputFilename: ""}
}
...
func (d Download) Go() error {
	...
	d.outputFilename = d.url[strings.LastIndex(d.url, "/")+1:]
	...
	timeToFinish := time.Since(startTime).Seconds()
	fmt.Printf("Done.\n\n")
	fmt.Printf("Average download speed: %dMB/s\n", int(float64(sizeInMB)/timeToFinish))
	fmt.Printf("Time to finish: %.2f seconds\n", timeToFinish)
...
func (d Download) MergeSections() error {
	// Create output file
	outputFile, err := os.OpenFile(d.outputFilename, os.O_CREATE|os.O_WRONLY|os.O_APPEND|os.O_TRUNC, os.ModePerm)
	if err != nil {
		return err
	}
